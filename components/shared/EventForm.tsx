...
const EventForm = ({ userId, type, event, eventId }: EventFormProps) => {
    ...
    async function onSubmit(values: z.infer<typeof eventFormSchema>) {
        ...
        if (files.length > 0) {
            const upladedImages = await startUpload(files);

            if (!upladedImages) {
                return
            }

            uploadImageUrl = upladedImages[0].url
        }
                ...
                <div className="flex flex-col gap-5 md:flex-row">
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem className="w-full">
                                <FormControl>
                                    <div className="flex-center h-[55px] rounded-full w-full overflow-hidden bg-gray-50 px-4 py-2">
                                        <Image
                ...
                <div className="flex flex-col gap-5 md:flex-row">
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem className="w-full">
                                <FormControl>
                                    <div className="flex-center h-[55px] rounded-full w-full overflow-hidden bg-grey-50 px-4 py-2">
                                        <Image
